...
//
// The session detail page and the task detail page both render the same
// underlying SDK conversation (each task stage IS a session). To keep the
// renderings consistent — token streaming, paired tool calls, thinking
// blocks, per-turn cost chips — they share this reducer + snapshot
// normalizer. Behavior is identical to the original SessionDetail
...
export function normalizeConversation(raw: unknown[]): {
  messages: ConversationMessage[];
  toolNames: Record<string, string>;
  toolIndexById: Record<string, number>;
} {
